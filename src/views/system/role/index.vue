    ...
    <div>
      <div v-if="showSearch" class="filter-container">
        <el-form :inline="true" :model="listQuery" label-width="auto">
          <el-form-item label="角色编码 ">
            <el-input v-model="listQuery.roleCode" placeholder="请输入角色编码 " />
          </el-form-item>
          <el-form-item label="角色名称 ">
            <el-input v-model="listQuery.roleName" placeholder="请输入角色名称 " />
          </el-form-item>
          <el-form-item label="角色简称 ">
            <el-input v-model="listQuery.roleAbbr" placeholder="请输入角色简称 " />
          </el-form-item>
          <el-form-item label="归属公司编号 ">
            <el-input v-model="listQuery.companyId" placeholder="请输入归属公司编号 " />
          </el-form-item>
          <el-form-item label="归属部门编号 ">
            <el-input v-model="listQuery.deptId" placeholder="请输入归属部门编号 " />
          </el-form-item>
      ...
      <div class="table-container">
        ...
        <el-table v-loading="listLoading" :data="list" size="mini" element-loading-text="Loading" fit border highlight-current-row>
          ...
          <el-table-column v-if="columns[0].visible" label="角色编号 " prop="roleId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[1].visible" label="角色编码 " prop="roleCode" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[2].visible" label="角色名称 " prop="roleName" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[3].visible" label="角色级别 " prop="roleLevel" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[4].visible" label="角色简称 " prop="roleAbbr" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[5].visible" label="角色别名 " prop="roleAliasName" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[6].visible" label="租户编号 " prop="tenantId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[7].visible" label="归属公司编号 " prop="companyId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[8].visible" label="归属部门编号 " prop="deptId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[9].visible" label="备注 " prop="remarks" align="center" sortable :show-overflow-tooltip="true" />
          ...
          <el-table-column v-if="columns[14].visible" label="更新时间" align="center">
            <template slot-scope="scope">
      ...
      <el-dialog :title="titleMap[dialogStatus]" :visible.sync="dialogVisible" width="40%" @close="handleDialogClose">
        <el-form ref="dataForm" :model="form" :rules="rules" label-width="auto" class="demo-ruleForm">
          ...
          <el-form-item v-if="true" label="角色级别 :" prop="roleLevel">
            <el-input
              v-model="form.roleLevel"
              placeholder="请输入角色级别 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="角色简称 :" prop="roleAbbr">
            <el-input
              v-model="form.roleAbbr"
              placeholder="请输入角色简称 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="角色别名 :" prop="roleAliasName">
            <el-input
          ...
          <el-form-item v-if="true" label="归属公司编号 :" prop="companyId">
            <el-input
              v-model="form.companyId"
              placeholder="请输入归属公司编号 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="归属部门编号 :" prop="deptId">
            <el-input
              v-model="form.deptId"
      ...
      <el-dialog :title="titleMap[dialogStatus]" :visible.sync="updateDialogVisible" width="40%" @close="handleDialogClose">
        <el-form ref="updateDataForm" :model="updateForm" :rules="updateRules" label-width="auto" class="demo-ruleForm">
          <el-form-item v-if="true" label="角色编号 :" prop="roleId">
            <el-input v-model="updateForm.roleId" disabled />
...
<script>
import { getRoleListPage, saveRole, updateRole, deleteById, download } from '@/api/system/role'
...
export default {
  ...
  methods: {
    getList() {
      ...
      getRoleListPage(this.listQuery).then(response => {
        ...
        if (valid) {
          this.$confirm('确定操作吗?', '提示', {
            ...
            if (_this.dialogStatus === 'create') {
              saveRole(_this.form).then(response => {
                if (response.code === 200) {
                  _this.getList()
              ...
              updateRole(_this.updateForm).then(response => {
                if (response.code === 200) {
                  _this.getList()
                  _this.$message.success(response.message)
