    ...
    <div>
      <div v-if="showSearch" class="filter-container">
        <el-form :inline="true" :model="listQuery" label-width="auto">
          <el-form-item label="部门编号 ">
            <el-input v-model="listQuery.deptId" placeholder="请输入部门编号 " />
          </el-form-item>
          <el-form-item label="部门编码 ">
            <el-input v-model="listQuery.deptCode" placeholder="请输入部门编码 " />
          </el-form-item>
          <el-form-item label="部门名称 ">
            <el-input v-model="listQuery.deptName" placeholder="请输入部门名称 " />
          </el-form-item>
          <el-form-item label="所属公司编码 ">
            <el-input v-model="listQuery.companyId" placeholder="请输入所属公司编码 " />
          </el-form-item>
          <el-form-item label="归属业务条线 ">
            <el-input v-model="listQuery.businessLine" placeholder="请输入归属业务条线 " />
          </el-form-item>
        </el-form>
      ...
      <div class="table-container">
        ...
        <el-table v-loading="listLoading" :data="list" size="mini" element-loading-text="Loading" fit border highlight-current-row>
          ...
          <el-table-column v-if="columns[0].visible" label="部门编号 " prop="deptId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[1].visible" label="部门编码 " prop="deptCode" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[2].visible" label="部门名称 " prop="deptName" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[3].visible" label="所属公司编码 " prop="companyId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[4].visible" label="所属公司编码 " prop="companyCode" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[5].visible" label="归属业务条线 " prop="businessLine" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[6].visible" label="所属租户 " prop="tenantId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[7].visible" label="上级部门编号 " prop="deptParentId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[8].visible" label="上级部门编码 " prop="deptParentCode" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[9].visible" label="部门层级 " prop="deptLevel" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[10].visible" label="部门类型 " prop="deptType" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[11].visible" label="部门类型编码 " prop="deptTypeCode" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[12].visible" label="备注 " prop="remark" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[13].visible" label="排序 " prop="deptOrder" align="center" sortable :show-overflow-tooltip="true" />
          ...
          <el-table-column v-if="columns[16].visible" label="操作人姓名 " prop="operatorName" align="center" sortable :show-overflow-tooltip="true" />
      ...
      <el-dialog :title="titleMap[dialogStatus]" :visible.sync="dialogVisible" width="40%" @close="handleDialogClose">
        <el-form ref="dataForm" :model="form" :rules="rules" label-width="auto" class="demo-ruleForm">
          ...
          <el-form-item v-if="true" label="部门编码 :" prop="deptCode">
            <el-input
              v-model="form.deptCode"
              placeholder="请输入部门编码 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="部门名称 :" prop="deptName">
            <el-input
              v-model="form.deptName"
              placeholder="请输入部门名称 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="所属公司编码 :" prop="companyId">
            <el-input
              v-model="form.companyId"
              placeholder="请输入所属公司编码 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="所属公司编码 :" prop="companyCode">
            <el-input
              v-model="form.companyCode"
              placeholder="请输入所属公司编码 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="归属业务条线 :" prop="businessLine">
            <el-input
              v-model="form.businessLine"
              placeholder="请输入归属业务条线 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="所属租户 :" prop="tenantId">
            <el-input
              v-model="form.tenantId"
              placeholder="请输入所属租户 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="上级部门编号 :" prop="deptParentId">
            <el-input
              v-model="form.deptParentId"
              placeholder="请输入上级部门编号 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="上级部门编码 :" prop="deptParentCode">
            <el-input
              v-model="form.deptParentCode"
              placeholder="请输入上级部门编码 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="部门层级 :" prop="deptLevel">
            <el-input
              v-model="form.deptLevel"
              placeholder="请输入部门层级 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="部门类型 :" prop="deptType">
            <el-input
              v-model="form.deptType"
              placeholder="请输入部门类型 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="部门类型编码 :" prop="deptTypeCode">
            <el-input
          ...
          <el-form-item v-if="true" label="排序 :" prop="deptOrder">
            <el-input
      ...
      <el-dialog :title="titleMap[dialogStatus]" :visible.sync="updateDialogVisible" width="40%" @close="handleDialogClose">
        <el-form ref="updateDataForm" :model="updateForm" :rules="updateRules" label-width="auto" class="demo-ruleForm">
          <el-form-item v-if="true" label="部门编号 :" prop="deptId">
            <el-input v-model="updateForm.deptId" disabled />
...
<script>
import { getDeptListPage, saveDept, updateDept, deleteById, download } from '@/api/system/dept'
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { key: 5, value: 'businessLine', label: '归属业务条线 ', visible: true },
        { key: 6, value: 'tenantId', label: '所属租户 ', visible: true },
        { key: 7, value: 'deptParentId', label: '上级部门编号 ', visible: true },
        { key: 8, value: 'deptParentCode', label: '上级部门编码 ', visible: true },
        { key: 9, value: 'deptLevel', label: '部门层级 ', visible: true },
        { key: 10, value: 'deptType', label: '部门类型 ', visible: true },
        { key: 11, value: 'deptTypeCode', label: '部门类型编码 ', visible: true },
  ...
  methods: {
    getList() {
      ...
      getDeptListPage(this.listQuery).then(response => {
        ...
        if (valid) {
          this.$confirm('确定操作吗?', '提示', {
            ...
            if (_this.dialogStatus === 'create') {
              saveDept(_this.form).then(response => {
                if (response.code === 200) {
                  _this.getList()
              ...
              updateDept(_this.updateForm).then(response => {
                if (response.code === 200) {
                  _this.getList()
                  _this.$message.success(response.message)
