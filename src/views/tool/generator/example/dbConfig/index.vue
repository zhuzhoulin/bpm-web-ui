    ...
    <div>
      <div v-if="showSearch" class="filter-container">
        <el-form :inline="true" :model="listQuery" label-width="auto">
          <el-form-item label="数据源名称 ">
            <el-input v-model="listQuery.dbName" placeholder="请输入数据源名称 " />
          </el-form-item>
        </el-form>
      ...
      <div class="table-container">
        ...
        <el-table v-loading="listLoading" :data="list" size="mini" element-loading-text="Loading" fit border highlight-current-row>
          ...
          <el-table-column v-if="columns[0].visible" label="数据源编号 " prop="dbId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[1].visible" label="数据源名称 " prop="dbName" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[2].visible" label="url " prop="dbUrl" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[3].visible" label="用户名 " prop="dbUsername" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[4].visible" label="密码 " prop="dbPassword" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[5].visible" label="数据源类型 " prop="dbType" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[6].visible" label="登记目录 " prop="dbCatalog" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[7].visible" label="数据源库名 " prop="dbSchemaName" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[8].visible" label="驱动名称 " prop="dbDriverName" align="center" sortable :show-overflow-tooltip="true" />
      ...
      <el-dialog :title="titleMap[dialogStatus]" :visible.sync="dialogVisible" width="40%" @close="handleDialogClose">
        <el-form ref="dataForm" :model="form" :rules="rules" label-width="auto" class="demo-ruleForm">
          <el-form-item v-if="true" label="数据源编号 :" prop="dbId">
            <el-input v-model="form.dbId" disabled />

          </el-form-item>
          <el-form-item v-if="true" label="数据源名称 :" prop="dbName">
            <el-input
              v-model="form.dbName"
              placeholder="请输入数据源名称 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="url :" prop="dbUrl">
            <el-input
              v-model="form.dbUrl"
              placeholder="请输入url "
            />
          </el-form-item>
          <el-form-item v-if="true" label="用户名 :" prop="dbUsername">
            <el-input
              v-model="form.dbUsername"
              placeholder="请输入用户名 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="密码 :" prop="dbPassword">
            <el-input
              v-model="form.dbPassword"
              placeholder="请输入密码 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="数据源类型 :" prop="dbType">
            <el-input
              v-model="form.dbType"
              placeholder="请输入数据源类型 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="登记目录 :" prop="dbCatalog">
            <el-input
              v-model="form.dbCatalog"
              placeholder="请输入登记目录 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="数据源库名 :" prop="dbSchemaName">
            <el-input
              v-model="form.dbSchemaName"
              placeholder="请输入数据源库名 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="驱动名称 :" prop="dbDriverName">
            <el-input
      ...
      <el-dialog :title="titleMap[dialogStatus]" :visible.sync="updateDialogVisible" width="40%" @close="handleDialogClose">
        <el-form ref="updateDataForm" :model="updateForm" :rules="updateRules" label-width="auto" class="demo-ruleForm">
          <el-form-item v-if="true" label="数据源编号 :" prop="dbId">
            <el-input v-model="updateForm.dbId" disabled />
...
<script>
import { getDbConfigListPage, saveDbConfig, updateDbConfig, deleteById, download } from '@/api/generator/dbConfig'
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { key: 11, value: 'validState', label: '状态 1 有效 0 失效', visible: true },
  ...
  methods: {
    getList() {
      ...
      getDbConfigListPage(this.listQuery).then(response => {
        ...
        if (valid) {
          this.$confirm('确定操作吗?', '提示', {
            ...
            if (_this.dialogStatus === 'create') {
              saveDbConfig(_this.form).then(response => {
                if (response.code === 200) {
                  _this.getList()
                  _this.$message.success(response.message)
              ...
              updateDbConfig(_this.updateForm).then(response => {
                if (response.code === 200) {
                  _this.getList()
                  _this.$message.success(response.message)
