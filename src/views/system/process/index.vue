    ...
    <div>
      <div v-if="showSearch" class="filter-container">
        <el-form :inline="true" :model="listQuery" label-width="auto">
          ...
          <el-form-item label="流程名称 ">
            <el-input v-model="listQuery.processName" placeholder="请输入流程名称 " />
          </el-form-item>
          <el-form-item label="流程简称 ">
            <el-input v-model="listQuery.processAbbr" placeholder="请输入流程简称 " />
          </el-form-item>
          <el-form-item label="流程状态 ">
            <el-select v-model="listQuery.processStatus" clearable filterable placeholder="请选择">
              <el-option
      ...
      <div class="table-container">
        ...
        <el-table v-loading="listLoading" :data="list" size="mini" element-loading-text="Loading" fit border highlight-current-row>
          ...
          <el-table-column v-if="columns[0].visible" label="流程编码 " prop="processId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[1].visible" label="流程key " prop="processKey" align="center" sortable />
          <el-table-column v-if="columns[2].visible" label="流程名称 " prop="processName" align="center" sortable />
          <el-table-column v-if="columns[3].visible" label="流程归属菜单 " prop="processMenuId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[4].visible" label="流程简称 " prop="processAbbr" align="center" sortable />
          <el-table-column v-if="columns[5].visible" label="流程分类 " prop="processType" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[6].visible" label="所属公司 " prop="companyId" align="center" sortable :show-overflow-tooltip="true">
            <template slot-scope="scope">
              <span>{{ companyMap[scope.row.companyId] }}</span>
            </template>
          </el-table-column>
          <el-table-column v-if="columns[7].visible" label="公司编码 " prop="companyCode" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[8].visible" label="排序 " prop="sort" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[9].visible" label="租户编号 " prop="tenantId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[10].visible" label="默认流程详细编号 " prop="processDetailId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[11].visible" label="流程状态 " prop="processStatus" align="center" sortable :show-overflow-tooltip="true">
            <template slot-scope="scope">
              <span>{{ dictMap2[scope.row.processStatus] }}</span>
            </template>
          </el-table-column>
          <el-table-column v-if="columns[12].visible" label="流程备注 " prop="remarks" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[13].visible" label="状态" prop="validState" align="center">
            <template slot-scope="scope">
          ...
          <el-table-column v-if="columns[14].visible" label="操作人工号 " prop="operatorId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[15].visible" label="操作人姓名 " prop="operatorName" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[16].visible" label="创建时间 " align="center">
            <template slot-scope="scope">
              <span>{{ scope.row.createTime }}</span>
            </template>
          </el-table-column>
          <el-table-column v-if="columns[17].visible" label="更新时间 " align="center">
            <template slot-scope="scope">
              <span>{{ scope.row.updateTime }}</span>
            </template>
          </el-table-column>
          <el-table-column align="center" label="操作" fixed="right" min-width="550px">
            <template slot-scope="scope">
              <el-button v-waves size="mini" type="primary" icon="el-icon-edit" plain @click="handleUpdate(scope.row)">编辑</el-button>
              <el-button v-waves type="warning" size="mini" icon="el-icon-setting" plain @click="showEditDiagramDialog(scope.row)">设计</el-button>
              <el-button v-waves type="success" size="mini" icon="el-icon-s-promotion" plain @click="publish(scope.row)">发布</el-button>
              <el-button v-waves type="info" size="mini" plain icon="el-icon-camera" @click="showProcessVersionDialog(scope.row)">版本</el-button>
              <el-button v-waves type="success" size="mini" icon="el-icon-view" plain @click="showDiagram(scope.row)">查看</el-button>
      ...
      <el-dialog :title="titleMap[dialogStatus]" :visible.sync="dialogVisible" width="40%" :before-close="handleClose" @close="handleDialogClose">
        <el-form ref="dataForm" :model="form" :rules="rules" label-width="auto" class="demo-ruleForm">
          <el-form-item v-if="true" label="流程编码 :" prop="processId">
            <el-input v-model="form.processId" disabled />
          ...
          <el-form-item v-if="true" label="流程名称 :" prop="processName">
            <el-input
          ...
          <el-form-item v-if="true" label="流程简称 :" prop="processAbbr">
            <el-input
          ...
          <el-form-item v-if="true" label="所属公司 :" prop="companyId">
            <el-select v-model="form.companyId" placeholder="请选择" @change="getCompanyById(form.companyId,'create')">
              <el-option
          ...
          <el-form-item v-if="true" label="流程备注 :" prop="remarks">
            <el-input
      ...
      <el-dialog :title="titleMap[dialogStatus]" :visible.sync="updateDialogVisible" width="40%" :before-close="handleClose" @close="handleDialogClose">
        <el-form ref="updateDataForm" :model="updateForm" :rules="updateRules" label-width="auto" class="demo-ruleForm">
          <el-form-item v-if="true" label="流程编码 :" prop="processId">
            <el-input v-model="updateForm.processId" disabled />
          ...
          <el-form-item v-if="true" label="所属公司 :" prop="companyId">
            <el-select v-model="updateForm.companyId" placeholder="请选择" @change="getCompanyById(updateForm.companyId,'update')">
              <el-option
                v-for="(item,index) in companyList"
                :key="index"
                :label="item.label"
          ...
          <el-form-item v-if="true" label="状态 :" prop="validState">
            <el-switch v-model="updateForm.validState" :active-value="1" :inactive-value="0" active-text="有效" inactive-text="无效" />
          </el-form-item>
          <el-form-item v-if="false" label="操作人工号 :" prop="operatorId">
            <el-input
...
<script>
import { getProcessListPage, saveProcess, updateProcess, deleteById, download } from '@/api/process/process'
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { key: 3, value: 'processMenuId', label: '流程归属菜单 ', visible: true },
        ...
        { key: 10, value: 'processDetailId', label: '默认流程详细编号 ', visible: true },
  ...
  methods: {
    ...
    getList() {
      ...
      getProcessListPage(this.listQuery).then(response => {
        ...
        if (valid) {
          this.$confirm('确定操作吗?', '提示', {
            ...
            if (_this.dialogStatus === 'create') {
              saveProcess(_this.form).then(response => {
                if (response.code === 200) {
                  _this.getList()
              ...
              updateProcess(_this.updateForm).then(response => {
                if (response.code === 200) {
                  _this.getList()
                  _this.$message.success(response.message)
