    ...
    <div>
      <div v-if="showSearch" class="filter-container">
        <el-form :inline="true" :model="listQuery" label-width="auto">
          ...
          <el-form-item label="模板内容 ">
            <el-input v-model="listQuery.templateContent" type="textarea" autosize placeholder="请输入模板内容 " />
          </el-form-item>
          <el-form-item label="模板类型 ">
            <el-input v-model="listQuery.templateType" placeholder="请输入模板类型 " />
          </el-form-item>
          <el-form-item label="自定义输入文件名称 ">
            <el-input v-model="listQuery.outputFileName" placeholder="请输入自定义输入文件名称 " />
          </el-form-item>
          <el-form-item label="模板路径 ">
            <el-input v-model="listQuery.templatePath" placeholder="请输入模板路径 " />
          </el-form-item>
          <el-form-item label="模板组编号 ">
            <el-input v-model="listQuery.templateGroupId" placeholder="请输入模板组编号 " />
          </el-form-item>
        </el-form>
      ...
      <div class="table-container">
        ...
        <el-table v-loading="listLoading" :data="list" size="mini" element-loading-text="Loading" fit border highlight-current-row>
          ...
          <el-table-column v-if="columns[1].visible" label="模板名称 " prop="templateName" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[2].visible" label="模板内容 " prop="templateContent" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[3].visible" label="模板类型 " prop="templateType" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[4].visible" label="自定义输入文件名称 " prop="outputFileName" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[5].visible" label="模板路径 " prop="templatePath" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[6].visible" label="模板组编号 " prop="templateGroupId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[7].visible" label="租户编号 " prop="tenantId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[8].visible" label="备注 " prop="remarks" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[9].visible" label="状态" prop="validState" align="center">
            <template slot-scope="scope">
          ...
          <el-table-column v-if="columns[10].visible" label="操作人工号 " prop="operatorId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[11].visible" label="操作人姓名 " prop="operatorName" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[12].visible" label="创建时间" align="center">
            <template slot-scope="scope">
              <span>{{ scope.row.createTime }}</span>
            </template>
          </el-table-column>
          <el-table-column v-if="columns[13].visible" label="更新时间" align="center">
            <template slot-scope="scope">
      ...
      <el-dialog :title="titleMap[dialogStatus]" :visible.sync="dialogVisible" width="40%" @close="handleDialogClose">
        <el-form ref="dataForm" :model="form" :rules="rules" label-width="auto" class="demo-ruleForm">
          ...
          <el-form-item v-if="true" label="模板内容 :" prop="templateContent">
            <el-input
          ...
          <el-form-item v-if="true" label="自定义输入文件名称 :" prop="outputFileName">
            <el-input
              v-model="form.outputFileName"
              placeholder="请输入自定义输入文件名称 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="模板路径 :" prop="templatePath">
            <el-input
              v-model="form.templatePath"
              placeholder="请输入模板路径 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="模板组编号 :" prop="templateGroupId">
            <el-input
              v-model="form.templateGroupId"
              placeholder="请输入模板组编号 "
            />
          </el-form-item>
      ...
      <el-dialog :title="titleMap[dialogStatus]" :visible.sync="updateDialogVisible" width="40%" @close="handleDialogClose">
        <el-form ref="updateDataForm" :model="updateForm" :rules="updateRules" label-width="auto" class="demo-ruleForm">
          ...
          <el-form-item v-if="true" label="模板内容 :" prop="templateContent">
            <el-input
              v-model="updateForm.templateContent"
              type="textarea"
              autosize
              placeholder="请输入模板内容 "
            />
          ...
          <el-form-item v-if="true" label="自定义输入文件名称 :" prop="outputFileName">
            <el-input
              v-model="updateForm.outputFileName"
              placeholder="请输入自定义输入文件名称 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="模板路径 :" prop="templatePath">
            <el-input
              v-model="updateForm.templatePath"
              placeholder="请输入模板路径 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="模板组编号 :" prop="templateGroupId">
...
<script>
import { getTemplateListPage, saveTemplate, updateTemplate, deleteById, download } from '@/api/generator/template'
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { key: 1, value: 'templateName', label: '模板名称', visible: true },
        { key: 2, value: 'templateContent', label: '模板内容', visible: true },
        { key: 3, value: 'templateType', label: '模板类型 1 后端 0 前端', visible: true },
        { key: 4, value: 'outputFileName', label: '自定义输入文件名称', visible: true },
        { key: 5, value: 'templatePath', label: '模板路径', visible: true },
        { key: 6, value: 'templateGroupId', label: '模板组编号', visible: true },
        { key: 7, value: 'tenantId', label: '租户编号', visible: true },
        { key: 8, value: 'remarks', label: '备注', visible: true },
        { key: 9, value: 'validState', label: '状态 1 有效 0 失效', visible: true },
      ...
      rules: {
        ...
        outputFileName: [
          { required: true, message: '请输入自定义输入文件名称', trigger: 'blur' }
        ],
        templatePath: [
          { required: true, message: '请输入模板路径', trigger: 'blur' }
        ],
        templateGroupId: [
          { required: true, message: '请输入模板组编号', trigger: 'blur' }
        ],
        tenantId: [
        ],
        remarks: [
  ...
  methods: {
    getList() {
      ...
      getTemplateListPage(this.listQuery).then(response => {
        ...
        if (valid) {
          this.$confirm('确定操作吗?', '提示', {
            ...
            if (_this.dialogStatus === 'create') {
              saveTemplate(_this.form).then(response => {
                if (response.code === 200) {
                  _this.getList()
                  _this.$message.success(response.message)
              ...
              updateTemplate(_this.updateForm).then(response => {
                if (response.code === 200) {
                  _this.getList()
                  _this.$message.success(response.message)
