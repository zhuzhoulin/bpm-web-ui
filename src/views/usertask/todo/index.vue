    ...
    <div class="container-info">
          ...
          <el-form-item :label="$t('userTaskList.applyStatus')">
            <el-select v-model="searchObject.applyStatus" :placeholder="$t('userTaskList.applyStatus')" filterable clearable>
              ...
              <!-- <el-option
                v-for="(item, index) in applyStatusOptions"
                :key="index"
                :label="$t('userTaskList.status'+item.value)"
                :value="item.value"
              /> -->
            </el-select>
          </el-form-item>
      ...
      <div v-loading="listLoading" class="content-container">
        ...
        >
          ...
          <el-table-column :label="$t('userTaskList.createTime')" align="center" width="160">
            <template slot-scope="scope">
          ...
          <el-table-column align="center" :label="$t('userTaskList.action')" width="110" class-name="small-padding fixed-width">
            <template slot-scope="scope">
              <el-row>
                <el-col>
                  <el-button type="primary" size="mini" plain @click="showDetail(scope.row)">{{ $t('userTaskList.approve') }}</el-button>
                </el-col>
              </el-row>
            </template>
          </el-table-column>
        ...
        <div style="margin-top: 20px">
          <el-button @click="batchCompleteTask">{{ $t('userTaskList.batchPass') }}</el-button>
        </div>
      </div>
...
import { getToDoList, batchCompleteTask } from '@/api/process/userTask'
...
export default {
  ...
  methods: {
    batchCompleteTask() {
      ...
      that.loading = true
      const param = {
        taskIds: that.multipleSelection,
        approveActionCode: 'approvePass',
        approveOpinion: '批量审批通过',
        businessData: null
      }

      if (that.multipleSelection === undefined || that.multipleSelection.length <= 0) {
        that.$message.error('请选择任务!')
        return
      }

      batchCompleteTask(param).then(response => {
    ...
    handleSelectionChange(val) {
      ...
      val.forEach(element => {
        multipleSelection.push(element.taskId)
      })
