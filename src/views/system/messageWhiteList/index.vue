      ...
      >
        ...
        >
          ...
          <el-form-item label="白名单类型 ">
            <el-input
              v-model="listQuery.whiteTypeCode"
              placeholder="请输入白名单类型 "
            />
          </el-form-item>
          <el-form-item label="白名单 ">
            <el-input
              v-model="listQuery.whiteList"
              placeholder="请输入白名单 "
            />
          </el-form-item>
...
<script>
import {
  getMessageWhiteListListPage,
  saveMessageWhiteList,
  updateMessageWhiteList,
  deleteById,
  download
} from '@/api/system/messageWhiteList'
...
export default {
  ...
  methods: {
    getList() {
      ...
      getMessageWhiteListListPage(this.listQuery)
        .then((response) => {
        ...
        if (valid) {
          this.$confirm('确定操作吗?', '提示', {
            ...
            if (_this.dialogStatus === 'create') {
              saveMessageWhiteList(_this.form)
                .then((response) => {
                  if (response.code === 200) {
                    _this.getList()
                    _this.$message.success(response.message)
              ...
              updateMessageWhiteList(_this.updateForm)
                .then((response) => {
                  if (response.code === 200) {
                    _this.getList()
                    _this.$message.success(response.message)
