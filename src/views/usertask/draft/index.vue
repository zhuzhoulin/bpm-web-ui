        ...
        >
          ...
          <el-table-column align="center" :label="$t('userTaskList.action')" width="110" class-name="small-padding fixed-width">
            <template slot-scope="scope">
              <el-row>
                <el-col>
                  <el-button type="primary" size="mini" plain @click="showDetail(scope.row)">{{ $t('userTaskList.view') }}</el-button>
                </el-col>
              </el-row>
            </template>
          </el-table-column>
...
import { getDraftList } from '@/api/process/userTask'
...
export default {
  ...
  methods: {
    ...
    queryList() {
      ...
      getDraftList(param).then(response => {
