    ...
    <div>
      <div v-if="showSearch" class="filter-container">
        <el-form :inline="true" :model="listQuery" label-width="auto">
          <el-form-item label="字典编码 ">
            <el-input v-model="listQuery.dictCode" placeholder="请输入字典编码 " clearable />
          </el-form-item>
          <el-form-item label="字典名称 ">
            <el-input v-model="listQuery.dictName" placeholder="请输入字典名称 " clearable />
          </el-form-item>
      ...
      <div class="table-container">
        ...
        <el-table v-loading="listLoading" :data="list" size="mini" element-loading-text="Loading" fit border highlight-current-row>
          ...
          <el-table-column v-if="columns[0].visible" label="主键 " prop="dictId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[1].visible" label="字典编码 " prop="dictCode" align="center" sortable />
          <el-table-column v-if="columns[2].visible" label="字典名称 " prop="dictName" align="center" sortable />
          <el-table-column v-if="columns[3].visible" label="树化 " prop="dictTree" align="center" sortable>
            <template slot-scope="scope">
          ...
          <el-table-column v-if="columns[5].visible" label="备注 " prop="remark" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[6].visible" label="状态" prop="validState" align="center">
            <template slot-scope="scope">
          ...
          <el-table-column v-if="columns[7].visible" label="操作人工号 " prop="operatorId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[8].visible" label="操作人姓名 " prop="operatorName" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[9].visible" label="操作时间" align="center">
            <template slot-scope="scope">
          ...
          <el-table-column align="center" label="操作" fixed="right" min-width="270px">
            <template slot-scope="scope">
              <el-button v-waves size="mini" type="primary" icon="el-icon-edit" plain @click="handleUpdate(scope.row)">编辑</el-button>
              <el-button type="success" size="mini" plain icon="el-icon-setting" @click="updateDictItem(scope.row)">更新</el-button>
      ...
      <el-dialog :title="titleMap[dialogStatus]" :visible.sync="dialogVisible" width="40%" @close="handleDialogClose">
        <el-form ref="dataForm" :model="form" :rules="rules" label-width="auto" class="demo-ruleForm">
          <el-form-item v-if="true" label="主键 :" prop="dictId">
            <el-input v-model="form.dictId" disabled />

          </el-form-item>
          <el-form-item v-if="true" label="字典编码 :" prop="dictCode">
            <el-input
              v-model="form.dictCode"
              placeholder="请输入字典编码 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="字典名称 :" prop="dictName">
            <el-input
              v-model="form.dictName"
              placeholder="请输入字典名称 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="树化 :" prop="dictTree">
            <el-switch
          ...
          <el-form-item v-if="false" label="更新时间 :" prop="updateTime">
            <el-date-picker v-model="form.updateTime" type="date" placeholder="请输入 " />
          </el-form-item>
          <el-form-item v-if="false" label="创建时间 :" prop="createTime">
            <el-date-picker v-model="form.createTime" type="date" placeholder="请输入 " />
          </el-form-item>
        </el-form>
        <span slot="footer" class="dialog-footer">
      ...
      <el-dialog :title="titleMap[dialogStatus]" :visible.sync="updateDialogVisible" width="40%" @close="handleDialogClose">
        <el-form ref="updateDataForm" :model="updateForm" :rules="updateRules" label-width="auto" class="demo-ruleForm">
          <el-form-item v-if="true" label="主键 :" prop="dictId">
            <el-input v-model="updateForm.dictId" disabled />
          ...
          <el-form-item v-if="false" label="更新时间 :" prop="updateTime">
            <el-date-picker v-model="updateForm.updateTime" type="date" placeholder="请输入 " />
          </el-form-item>
          <el-form-item v-if="false" label="创建时间 :" prop="createTime">
            <el-date-picker v-model="updateForm.createTime" type="date" placeholder="请输入 " />
          </el-form-item>
        </el-form>
        <span slot="footer" class="dialog-footer">
      ...
      >
        <div class="custom_drawer__content">
          <dictItemList ref="dictItem" :dict-info.sync="showDictItemDialog.data" @queryList="getList" @close="closeUpdateDictItem" />
        </div>
      </el-drawer>
    </div>
  </div>
...
<script>
import { getDictListPage, saveDict, updateDict, deleteById, download } from '@/api/system/dict'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getDictListPage(this.listQuery).then(response => {
        ...
        if (valid) {
          this.$confirm('确定操作吗?', '提示', {
            ...
            if (_this.dialogStatus === 'create') {
              saveDict(_this.form).then(response => {
                if (response.code === 200) {
                  _this.getList()
              ...
              updateDict(_this.updateForm).then(response => {
                if (response.code === 200) {
                  _this.getList()
                  _this.$message.success(response.message)
