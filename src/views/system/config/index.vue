    ...
    <div>
      <div v-if="showSearch" class="filter-container">
        <el-form :inline="true" :model="listQuery" label-width="auto">
          <el-form-item label="配置编码 ">
            <el-input v-model="listQuery.configId" placeholder="请输入配置编码 " />
          </el-form-item>
          <el-form-item label="模板编号 ">
            <el-input v-model="listQuery.configCode" placeholder="请输入模板编号 " />
          </el-form-item>
          <el-form-item label="模板名称 ">
            <el-input v-model="listQuery.configName" placeholder="请输入模板名称 " />
          </el-form-item>
          <el-form-item label="模板编号 ">
            <el-input v-model="listQuery.templateId" placeholder="请输入模板编号 " />
          </el-form-item>
          <el-form-item label="模板key ">
            <el-input v-model="listQuery.configKey" placeholder="请输入模板key " />
          </el-form-item>
      ...
      <div class="table-container">
        ...
        <el-table v-loading="listLoading" :data="list" size="mini" element-loading-text="Loading" fit border highlight-current-row>
          ...
          <el-table-column v-if="columns[0].visible" label="配置编码 " prop="configId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[1].visible" label="模板编号 " prop="configCode" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[2].visible" label="模板名称 " prop="configName" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[3].visible" label="模板编号 " prop="templateId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[4].visible" label="模板key " prop="configKey" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[5].visible" label="模板值 " prop="configValue" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[6].visible" label="模板类型 " prop="configType" align="center" sortable :show-overflow-tooltip="true" />
      ...
      <el-dialog :title="titleMap[dialogStatus]" :visible.sync="dialogVisible" width="40%" @close="handleDialogClose">
        <el-form ref="dataForm" :model="form" :rules="rules" label-width="auto" class="demo-ruleForm">
          <el-form-item v-if="true" label="配置编码 :" prop="configId">
            <el-input v-model="form.configId" disabled />

          </el-form-item>
          <el-form-item v-if="true" label="模板编号 :" prop="configCode">
            <el-input
              v-model="form.configCode"
              placeholder="请输入模板编号 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="模板名称 :" prop="configName">
            <el-input
          ...
          <el-form-item v-if="true" label="模板key :" prop="configKey">
            <el-input
              v-model="form.configKey"
              placeholder="请输入模板key "
            />
          </el-form-item>
          <el-form-item v-if="true" label="模板值 :" prop="configValue">
            <el-input
              v-model="form.configValue"
              placeholder="请输入模板值 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="模板类型 :" prop="configType">
            <el-input
...
<script>
import { getConfigListPage, saveConfig, updateConfig, deleteById, download } from '@/api/system/config'
...
export default {
  ...
  methods: {
    getList() {
      ...
      getConfigListPage(this.listQuery).then(response => {
        ...
        if (valid) {
          this.$confirm('确定操作吗?', '提示', {
            ...
            if (_this.dialogStatus === 'create') {
              saveConfig(_this.form).then(response => {
                if (response.code === 200) {
                  _this.getList()
              ...
              updateConfig(_this.updateForm).then(response => {
                if (response.code === 200) {
                  _this.getList()
                  _this.$message.success(response.message)
