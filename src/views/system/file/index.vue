    ...
    <div>
      <div v-if="showSearch" class="filter-container">
        <el-form :inline="true" :model="listQuery" label-width="auto">
          <el-form-item label="编号 ">
            <el-input v-model="listQuery.fileId" placeholder="请输入编号 " />
          </el-form-item>
          <el-form-item label="文件实例编号 ">
            <el-input v-model="listQuery.fileInstanceCode" placeholder="请输入文件实例编号 " />
          </el-form-item>
          <el-form-item label="模板文件名称 ">
            <el-input v-model="listQuery.fileName" placeholder="请输入模板文件名称 " />
          </el-form-item>
          <el-form-item label="所属系统 ">
            <el-input v-model="listQuery.fileSystemCode" placeholder="请输入所属系统 " />
          </el-form-item>
          <el-form-item label="所属平台 ">
            <el-input v-model="listQuery.fileplatform" placeholder="请输入所属平台 " />
          </el-form-item>
          <el-form-item label="文件所属人员 ">
            <el-input v-model="listQuery.fileOwnerId" placeholder="请输入文件所属人员 " />
          </el-form-item>
          <el-form-item label="文件所属人姓名 ">
            <el-input v-model="listQuery.fileOwnerName" placeholder="请输入文件所属人姓名 " />
          </el-form-item>
        </el-form>
      ...
      <div class="table-container">
        ...
        <el-table v-loading="listLoading" :data="list" size="mini" element-loading-text="Loading" fit border highlight-current-row>
          ...
          <el-table-column v-if="columns[0].visible" label="编号 " prop="fileId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[1].visible" label="文件实例编号 " prop="fileInstanceCode" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[2].visible" label="模板文件名称 " prop="fileName" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[3].visible" label="模板文件名称 " prop="fileMd5Name" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[4].visible" label="文件路径 " prop="filePath" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[5].visible" label="文件大小 " prop="fileSize" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[6].visible" label="文件后缀 " prop="fileExtend" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[7].visible" label="状态 " prop="fileStatus" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[8].visible" label="所属系统 " prop="fileSystemCode" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[9].visible" label="所属平台 " prop="fileplatform" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[10].visible" label="所属服务名称 " prop="fileServiceName" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[11].visible" label="所属于哪个方法 " prop="fileMethodName" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[12].visible" label="文件所属人员 " prop="fileOwnerId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[13].visible" label="文件所属人姓名 " prop="fileOwnerName" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[14].visible" label="租户编号 " prop="tenantId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[15].visible" label="错误日志 " prop="errorMessage" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[16].visible" label="状态" prop="validState" align="center">
            <template slot-scope="scope">
          ...
          <el-table-column v-if="columns[17].visible" label="操作人工号 " prop="operatorId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[18].visible" label="操作人姓名 " prop="operatorName" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[19].visible" label="创建时间" align="center">
            <template slot-scope="scope">
              <span>{{ scope.row.createTime }}</span>
            </template>
          </el-table-column>
          <el-table-column v-if="columns[20].visible" label="更新时间" align="center">
            <template slot-scope="scope">
      ...
      <el-dialog :title="titleMap[dialogStatus]" :visible.sync="dialogVisible" width="40%" @close="handleDialogClose">
        <el-form ref="dataForm" :model="form" :rules="rules" label-width="auto" class="demo-ruleForm">
          <el-form-item v-if="true" label="编号 :" prop="fileId">
            <el-input v-model="form.fileId" disabled />

          </el-form-item>
          <el-form-item v-if="true" label="文件实例编号 :" prop="fileInstanceCode">
            <el-input
              v-model="form.fileInstanceCode"
              placeholder="请输入文件实例编号 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="模板文件名称 :" prop="fileName">
            <el-input
              v-model="form.fileName"
              placeholder="请输入模板文件名称 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="模板文件名称 :" prop="fileMd5Name">
            <el-input
              v-model="form.fileMd5Name"
              placeholder="请输入模板文件名称 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="文件路径 :" prop="filePath">
            <el-input
              v-model="form.filePath"
              placeholder="请输入文件路径 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="文件大小 :" prop="fileSize">
            <el-input
              v-model="form.fileSize"
              placeholder="请输入文件大小 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="文件后缀 :" prop="fileExtend">
            <el-input
              v-model="form.fileExtend"
              placeholder="请输入文件后缀 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="状态 :" prop="fileStatus">
            <el-input
              v-model="form.fileStatus"
              placeholder="请输入状态 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="所属系统 :" prop="fileSystemCode">
            <el-input
              v-model="form.fileSystemCode"
              placeholder="请输入所属系统 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="所属平台 :" prop="fileplatform">
            <el-input
              v-model="form.fileplatform"
              placeholder="请输入所属平台 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="所属服务名称 :" prop="fileServiceName">
            <el-input
              v-model="form.fileServiceName"
              placeholder="请输入所属服务名称 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="所属于哪个方法 :" prop="fileMethodName">
            <el-input
              v-model="form.fileMethodName"
              placeholder="请输入所属于哪个方法 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="文件所属人员 :" prop="fileOwnerId">
            <el-input
              v-model="form.fileOwnerId"
              placeholder="请输入文件所属人员 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="文件所属人姓名 :" prop="fileOwnerName">
            <el-input
          ...
          <el-form-item v-if="true" label="错误日志 :" prop="errorMessage">
            <el-input
          ...
          <el-form-item v-if="false" label="最后一次更新时间 :" prop="updateTime">
            <el-date-picker v-model="form.updateTime" type="date" placeholder="请输入最后一次更新时间 " />
          </el-form-item>
        </el-form>
        <span slot="footer" class="dialog-footer">
      ...
      <el-dialog :title="titleMap[dialogStatus]" :visible.sync="updateDialogVisible" width="40%" @close="handleDialogClose">
        <el-form ref="updateDataForm" :model="updateForm" :rules="updateRules" label-width="auto" class="demo-ruleForm">
          <el-form-item v-if="true" label="编号 :" prop="fileId">
            <el-input v-model="updateForm.fileId" disabled />
          ...
          <el-form-item v-if="false" label="最后一次更新时间 :" prop="updateTime">
            <el-date-picker v-model="updateForm.updateTime" type="date" placeholder="请输入最后一次更新时间 " />
          </el-form-item>
        </el-form>
        <span slot="footer" class="dialog-footer">
...
<script>
import { getFileListPage, saveFile, updateFile, deleteById, download } from '@/api/system/file'
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { key: 1, value: 'fileInstanceCode', label: '文件实例编号 ', visible: true },
        ...
        { key: 10, value: 'fileServiceName', label: '所属服务名称 ', visible: true },
        { key: 11, value: 'fileMethodName', label: '所属于哪个方法 ', visible: true },
        { key: 12, value: 'fileOwnerId', label: '文件所属人员 ', visible: true },
        { key: 13, value: 'fileOwnerName', label: '文件所属人姓名 ', visible: true },
        ...
        { key: 20, value: 'updateTime', label: '最后一次更新时间 ', visible: true }
      ],
      dialogVisible: false,
  ...
  methods: {
    getList() {
      ...
      getFileListPage(this.listQuery).then(response => {
        ...
        if (valid) {
          this.$confirm('确定操作吗?', '提示', {
            ...
            if (_this.dialogStatus === 'create') {
              saveFile(_this.form).then(response => {
                if (response.code === 200) {
                  _this.getList()
              ...
              updateFile(_this.updateForm).then(response => {
                if (response.code === 200) {
                  _this.getList()
                  _this.$message.success(response.message)
