    ...
    <div>
      <div v-if="showSearch" class="filter-container">
        <el-form :inline="true" :model="listQuery" label-width="auto">
          <el-form-item label="操作用户工号 ">
            <el-input v-model="listQuery.userId" placeholder="请输入操作用户工号 " />
          </el-form-item>
          <el-form-item label="操作用户 ">
            <el-input v-model="listQuery.nickname" placeholder="请输入操作用户 " />
          </el-form-item>
          <el-form-item label="追踪编号 ">
            <el-input v-model="listQuery.traceId" placeholder="请输入追踪编号 " />
          </el-form-item>
          <el-form-item label="业务类型 ">
            <el-input v-model="listQuery.businessType" placeholder="请输入业务类型 " />
          </el-form-item>
          <el-form-item label="模块名称 ">
            <el-input v-model="listQuery.moduleName" placeholder="请输入模块名称 " />
          </el-form-item>
          <el-form-item label="请求方法名称 ">
            <el-input v-model="listQuery.methodName" placeholder="请输入请求方法名称 " />
          </el-form-item>
          ...
          <el-form-item label="操作类别 ">
            <el-input v-model="listQuery.operatorType" placeholder="请输入操作类别 " />
          </el-form-item>
          <el-form-item label="状态码 ">
            <el-input v-model="listQuery.code" placeholder="请输入状态码 " />
          </el-form-item>
        </el-form>
      ...
      <div class="table-container">
        ...
        <el-table v-loading="listLoading" :data="list" size="mini" element-loading-text="Loading" fit border highlight-current-row>
          ...
          <el-table-column v-if="columns[0].visible" label="日志编号 " prop="logId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[1].visible" label="操作用户工号 " prop="userId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[2].visible" label="操作用户 " prop="nickname" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[3].visible" label="追踪编号 " prop="traceId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[4].visible" label="业务类型 " prop="businessType" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[5].visible" label="模块名称 " prop="moduleName" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[6].visible" label="请求类名称 " prop="className" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[7].visible" label="请求方法名称 " prop="methodName" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[8].visible" label="请求方式 " prop="requestMethod" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[9].visible" label="操作地址 " prop="ipAddress" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[10].visible" label="详细地址 " prop="detailAddress" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[11].visible" label="浏览器 " prop="browser" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[12].visible" label="操作系统 " prop="os" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[13].visible" label="操作类别 " prop="operatorType" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[14].visible" label="请求参数 " prop="param" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[15].visible" label="状态码 " prop="code" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[16].visible" label="返回描述 " prop="message" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[17].visible" label="返回值 " prop="data" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[18].visible" label="备注 " prop="remarks" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[19].visible" label="状态" prop="validState" align="center">
            <template slot-scope="scope">
          ...
          <el-table-column v-if="columns[20].visible" label="操作人工号 " prop="operatorId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[21].visible" label="操作人姓名 " prop="operatorName" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[22].visible" label="创建时间" align="center">
            <template slot-scope="scope">
              <span>{{ scope.row.createTime }}</span>
            </template>
          </el-table-column>
          <el-table-column v-if="columns[23].visible" label="更新时间" align="center">
            <template slot-scope="scope">
      ...
      <el-dialog :title="titleMap[dialogStatus]" :visible.sync="dialogVisible" width="40%" @close="handleDialogClose">
        <el-form ref="dataForm" :model="form" :rules="rules" label-width="auto" class="demo-ruleForm">
          <el-form-item v-if="true" label="日志编号 :" prop="logId">
            <el-input v-model="form.logId" disabled />

          </el-form-item>
          <el-form-item v-if="true" label="操作用户工号 :" prop="userId">
            <el-input
              v-model="form.userId"
              placeholder="请输入操作用户工号 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="操作用户 :" prop="nickname">
            <el-input
              v-model="form.nickname"
              placeholder="请输入操作用户 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="追踪编号 :" prop="traceId">
            <el-input
              v-model="form.traceId"
              placeholder="请输入追踪编号 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="业务类型 :" prop="businessType">
            <el-input
          ...
          <el-form-item v-if="true" label="请求类名称 :" prop="className">
            <el-input
              v-model="form.className"
              placeholder="请输入请求类名称 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="请求方法名称 :" prop="methodName">
            <el-input
              v-model="form.methodName"
              placeholder="请输入请求方法名称 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="请求方式 :" prop="requestMethod">
            <el-input
              v-model="form.requestMethod"
              placeholder="请输入请求方式 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="操作地址 :" prop="ipAddress">
            <el-input
              v-model="form.ipAddress"
              placeholder="请输入操作地址 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="详细地址 :" prop="detailAddress">
            <el-input
              v-model="form.detailAddress"
              placeholder="请输入详细地址 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="浏览器 :" prop="browser">
            <el-input
              v-model="form.browser"
              placeholder="请输入浏览器 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="操作系统 :" prop="os">
            <el-input
              v-model="form.os"
              placeholder="请输入操作系统 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="操作类别 :" prop="operatorType">
            <el-input
              v-model="form.operatorType"
              placeholder="请输入操作类别 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="请求参数 :" prop="param">
            <el-input
              v-model="form.param"
              placeholder="请输入请求参数 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="状态码 :" prop="code">
            <el-input
              v-model="form.code"
              placeholder="请输入状态码 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="返回描述 :" prop="message">
            <el-input
              v-model="form.message"
              placeholder="请输入返回描述 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="返回值 :" prop="data">
            <el-input
              v-model="form.data"
              type="textarea"
              autosize
...
<script>
import { getOperatorLogListPage, saveOperatorLog, updateOperatorLog, deleteById, download } from '@/api/system/operatorLog'
...
export default {
  ...
  methods: {
    getList() {
      ...
      getOperatorLogListPage(this.listQuery).then(response => {
        ...
        if (valid) {
          this.$confirm('确定操作吗?', '提示', {
            ...
            if (_this.dialogStatus === 'create') {
              saveOperatorLog(_this.form).then(response => {
                if (response.code === 200) {
                  _this.getList()
              ...
              updateOperatorLog(_this.updateForm).then(response => {
                if (response.code === 200) {
                  _this.getList()
                  _this.$message.success(response.message)
