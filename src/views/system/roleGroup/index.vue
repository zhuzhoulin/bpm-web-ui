    ...
    <div>
      <div v-if="showSearch" class="filter-container">
        <el-form :inline="true" :model="listQuery" label-width="auto">
          <el-form-item label="角色组编码 ">
            <el-input v-model="listQuery.roleGroupCode" placeholder="请输入角色组编码 " />
          </el-form-item>
          <el-form-item label="角色组名称 ">
            <el-input v-model="listQuery.roleGroupName" placeholder="请输入角色组名称 " />
          </el-form-item>
          <el-form-item label="所属条线 ">
            <el-input v-model="listQuery.businessLine" placeholder="请输入所属条线 " />
          </el-form-item>
      ...
      <div class="table-container">
        ...
        <el-table v-loading="listLoading" :data="list" size="mini" element-loading-text="Loading" fit border highlight-current-row>
          ...
          <el-table-column v-if="columns[0].visible" label=" " prop="roleGroupId" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[1].visible" label="角色组编码 " prop="roleGroupCode" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[2].visible" label="角色组名称 " prop="roleGroupName" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[3].visible" label="角色组简称 " prop="roleGroupAbbr" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[4].visible" label="所属条线 " prop="businessLine" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[5].visible" label="角色组等级 " prop="roleGroupLevel" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[6].visible" label="角色组类别 " prop="roleGroupType" align="center" sortable :show-overflow-tooltip="true" />
      ...
      <el-dialog :title="titleMap[dialogStatus]" :visible.sync="dialogVisible" width="40%" @close="handleDialogClose">
        <el-form ref="dataForm" :model="form" :rules="rules" label-width="auto" class="demo-ruleForm">
          <el-form-item v-if="true" label=" :" prop="roleGroupId">
            <el-input v-model="form.roleGroupId" disabled />

          </el-form-item>
          <el-form-item v-if="true" label="角色组编码 :" prop="roleGroupCode">
            <el-input
          ...
          <el-form-item v-if="true" label="角色组简称 :" prop="roleGroupAbbr">
            <el-input
              v-model="form.roleGroupAbbr"
              placeholder="请输入角色组简称 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="所属条线 :" prop="businessLine">
            <el-input
              v-model="form.businessLine"
              placeholder="请输入所属条线 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="角色组等级 :" prop="roleGroupLevel">
            <el-input
              v-model="form.roleGroupLevel"
              placeholder="请输入角色组等级 "
            />
          </el-form-item>
          <el-form-item v-if="true" label="角色组类别 :" prop="roleGroupType">
            <el-input
      ...
      <el-dialog :title="titleMap[dialogStatus]" :visible.sync="updateDialogVisible" width="40%" @close="handleDialogClose">
        <el-form ref="updateDataForm" :model="updateForm" :rules="updateRules" label-width="auto" class="demo-ruleForm">
          <el-form-item v-if="true" label=" :" prop="roleGroupId">
            <el-input v-model="updateForm.roleGroupId" disabled />
...
<script>
import { getRoleGroupListPage, saveRoleGroup, updateRoleGroup, deleteById, download } from '@/api/system/roleGroup'
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { key: 5, value: 'roleGroupLevel', label: '角色组等级 ', visible: true },
  ...
  methods: {
    getList() {
      ...
      getRoleGroupListPage(this.listQuery).then(response => {
        ...
        if (valid) {
          this.$confirm('确定操作吗?', '提示', {
            ...
            if (_this.dialogStatus === 'create') {
              saveRoleGroup(_this.form).then(response => {
                if (response.code === 200) {
                  _this.getList()
              ...
              updateRoleGroup(_this.updateForm).then(response => {
                if (response.code === 200) {
                  _this.getList()
                  _this.$message.success(response.message)
