    ...
    <div>
      ...
      <div class="table-container">
        ...
        <el-table v-loading="listLoading" :data="list" size="mini" element-loading-text="Loading" fit border highlight-current-row>
          ...
          <el-table-column v-if="columns[0].visible" label="GUID主键 " prop="tempalteId" align="center" sortable :show-overflow-tooltip="true" />
          ...
          <el-table-column v-if="columns[3].visible" label="模板文件名称 " prop="fileName" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[4].visible" label="模板文件名称 " prop="fileMd5Name" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[5].visible" label="文件后缀 " prop="fileExtend" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[6].visible" label="文件路径 " prop="filePath" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[7].visible" label="文件大小 " prop="fileSize" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[8].visible" label="备注 " prop="remarks" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[9].visible" label="排序值 " prop="sort" align="center" sortable :show-overflow-tooltip="true" />
          <el-table-column v-if="columns[10].visible" label="状态" prop="validState" align="center">
            <template slot-scope="scope">
          ...
          <el-table-column v-if="columns[13].visible" label="创建时间" align="center">
            <template slot-scope="scope">
              <span>{{ scope.row.createTime }}</span>
            </template>
          </el-table-column>
          <el-table-column v-if="columns[14].visible" label="最后一次更新时间" align="center">
            <template slot-scope="scope">
      ...
      <el-dialog :title="titleMap[dialogStatus]" :visible.sync="dialogVisible" width="40%" @close="handleDialogClose">
        <el-form ref="dataForm" :model="form" :rules="rules" label-width="auto" class="demo-ruleForm">
          <el-form-item v-if="true" label="GUID主键 :" prop="tempalteId">
            <el-input v-model="form.tempalteId" disabled />
          ...
          <el-form-item v-if="false" label="操作人工号 :" prop="operatorId">
            <el-switch v-model="form.operatorId" :active-value="1" :inactive-value="0" />
          </el-form-item>
      ...
      <el-dialog :title="titleMap[dialogStatus]" :visible.sync="updateDialogVisible" width="40%" @close="handleDialogClose">
        <el-form ref="updateDataForm" :model="updateForm" :rules="updateRules" label-width="auto" class="demo-ruleForm">
          <el-form-item v-if="true" label="GUID主键 :" prop="tempalteId">
            <el-input v-model="updateForm.tempalteId" disabled />
          ...
          <el-form-item v-if="false" label="操作人工号 :" prop="operatorId">
            <el-switch v-model="updateForm.operatorId" :active-value="1" :inactive-value="0" />
          </el-form-item>
          <el-form-item v-if="false" label="操作人姓名 :" prop="operatorName">
            <el-input
...
<script>
import { getFileTemplateListPage, saveFileTemplate, updateFileTemplate, deleteById, download } from '@/api/system/fileTemplate'
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { key: 14, value: 'updateTime', label: '最后一次更新时间 ', visible: true }
      ],
      dialogVisible: false,
  ...
  methods: {
    getList() {
      ...
      getFileTemplateListPage(this.listQuery).then(response => {
        ...
        if (valid) {
          this.$confirm('确定操作吗?', '提示', {
            ...
            if (_this.dialogStatus === 'create') {
              saveFileTemplate(_this.form).then(response => {
                if (response.code === 200) {
                  _this.getList()
              ...
              updateFileTemplate(_this.updateForm).then(response => {
                if (response.code === 200) {
                  _this.getList()
                  _this.$message.success(response.message)
